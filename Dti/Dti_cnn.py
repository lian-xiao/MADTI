...
class GCNStem(nn.Module):
    ...
    def __init__(self, in_feature):
        ...
        self.gelu1 = nn.GELU()
        self.bn1 = nn.BatchNorm1d(in_feature,eps=1e-2)
        self.linear2 = nn.Linear(in_feature,in_feature)
        self.gelu2 = nn.GELU()
        self.bn2 = nn.BatchNorm1d(in_feature,eps=1e-2)
...
class Dti_cnn(nn.Module):
    def __init__(self, d_embedding, p_embedding, config):
        ...
        self.drug_extractor = MolecularGCN_muti(in_feats=config.d_emb, dim_embedding=config.d_stem_channel,
                                                padding=True,
                                                hidden_feats=config.d_channels,
                                                out_feats=config.d_out_channel,stem=config.d_stem)
        self.prot_extractor = ProCnn_muti(p_embedding, config.p_emb, config.p_stem_channel, config.p_channels,
                                             config.p_out_channel,config.stem_kernel,stem=config.d_stem)
        self.bcn = weight_norm(
            BANLayer(v_dim=config.d_out_channel, q_dim=config.p_out_channel, h_dim=config.out_hidden_size, h_out=2),
            name='h_mat', dim=None)

        self.pred = MLPDecoder(config.out_hidden_size, 512, 128, config.binary)
...
class Dti_cnn_mutiout(nn.Module):
    def __init__(self,d_embedding, p_embedding, config):
        ...
        #self.bcn_gate = GAU(dim=config.out_hidden_size, query_key_dim=128)
        if config.gate:
            if hasattr(config,'norm'):
                self.bcn_gate = GAU(dim=config.out_hidden_size,query_key_dim=128,norm=config.norm)
            else:
                self.bcn_gate = GAU(dim=config.out_hidden_size, query_key_dim=128)
        else:
            self.bcn_gate = nn.Identity()
        self.pred = MLPDecoder(config.out_hidden_size, 512, 128,config.binary)
...
# total_count = df['Y'].count()
# category_proportions = (df['Y'].value_counts(normalize=True) * 100).round(2)
